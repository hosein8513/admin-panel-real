...
const Login = () => {
    ...
    const onSubmit = async (value, navigate) => {
        try {
            ...
            {formik => (
                <div className="w-full max-w-md bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg p-6">
                    ...
                    <Form className="space-y-4 w-full">
                        ...
                        <div className='p-4'>
                            <button type="submit" className={`w-full py-2 rounded-3 text-center bg-indigo-600 text-white font-semibold shadow hover:bg-indigo-700 transition ${!disable ? 'disabled:bg-indigo-600/40' : ''}`} onClick={() => setdisable(false)}>
                                {!disable ? "لطفا صبر کنید ..." : "ورود"}
                            </button>
                        </div>
                    </Form>
                </div>
